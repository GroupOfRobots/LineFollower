...
//
//****************************************************************************//
LSM6DS3Core::LSM6DS3Core( uint8_t busType, uint8_t inputArg) : commInterface(I2C_MODE), I2CAddress(0x6B), chipSelectPin(1)
{
...
status_t LSM6DS3Core::beginCore(void)
{
	...
	case SPI_MODE:
		...
		bcm2835_gpio_fsel(BCM2835_SPI_CS1, BCM2835_GPIO_FSEL_OUTP);
...
//
//****************************************************************************//
status_t LSM6DS3Core::readRegisterRegion(uint8_t *outputPointer , uint8_t offset, uint8_t len)
{
	status_t returnError = IMU_SUCCESS;
	...
	case SPI_MODE:
		...
		dataPacket[0] = (offset | 0x80);
		bcm2835_spi_transfern(dataPacket, 1); //Ored with "read request" bit
		while ( i < length ) // slave may send less than requested
		{
			dataPacket[0] = 0x00;
			bcm2835_spi_transfern(dataPacket, 1); // receive a byte as character
			c = dataPacket[0];
			if( c == 0xFF )
			{
				//May have problem
